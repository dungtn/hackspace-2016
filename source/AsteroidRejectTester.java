...
{
    static final String PATH_TO_RAW_FILES = "data/raw_files";
    static final String PATH_TO_IMAGES_FOLDER = "data/images";
    ...
    static final String PATH_SEP = File.separator;
...
//    }
    ...
    public static void main(String[] args) throws Exception {
    	...
    	AsteroidRejectTester a = new AsteroidRejectTester();

    	File folder = new File(PATH_TO_RAW_FILES);
    	...
    	File outRootDir = new File(PATH_TO_IMAGES_FOLDER);
    	if (!outRootDir.exists()) {
    	    outRootDir.mkdirs();
    	}

    	if (outRootDir.exists()) {
    	    // String pathToRoot = outRootDir.getName();
    	    ArrayList<Integer> raw = new ArrayList<Integer>();
            for (File file : listFiles) {
                String filePath = file.getPath();
                String pathToOutput = getPathToOutput(PATH_TO_IMAGES_FOLDER, file.getName());

                raw.clear();
                a.loadRawImage(filePath, raw);

                int n = raw.size()/(4*64*64);
                for (int i = 0; i < n; i++) {
                    // outFileName = pathToOutput/out{0}.png
                    String outFileName = String.format("%sout%d.png", pathToOutput, i);
                    a.visualize(raw, i*4*64*64, outFileName);
                }
...
//
////    	ArrayList<Integer> raw = new ArrayList<Integer>();
//        a.loadRawImage(fileName, raw);
//
//        int n = raw.size()/(4*64*64);
//        for (int i = 0; i < n; i++) {
//            String fileOut = "out" + i + ".png";
//            a.visualize(raw, i*4*64*64, fileOut);
//        }
...
//        }
    ...
    static String getPathToOutput(String pathToRoot, String fileName) {

        String outDirChildName = fileName.replace(".raw", PATH_SEP);
        // pathToOutput = pathToRoot/outChildDirName/
        String pathToOutput = pathToRoot + PATH_SEP + outDirChildName;

        File outDir = new File(pathToOutput);
        if (!outDir.exists()) {
          outDir.mkdir();
        }
